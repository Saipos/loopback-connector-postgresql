...
// Dedup do log [PG-OVERRIDE]: 1 linha por application_name por processo (override on OU off).
// Valida na task qual fluxo cada datasource pegou -- override=on (entrou na logica custom)
// ou override=off (passthrough base, comportamento default intacto). Independe de env.
...
class ConnectionParametersWithKeepalive extends BaseConnectionParameters {
  ...
  getLibpqConnectionString(cb) {
    ...
    super.getLibpqConnectionString(function(err, conString) {
      ...
      // Sem override: devolve a string base INTACTA. Loga 1x/app que ficou no fluxo
      // BASE (passthrough) -- prova pro time que NAO entramos no fluxo de override e
      // nao mudamos o comportamento default do connector. Blindado.
      if (!self._override) {
        try {
          const appOff = self.application_name || 'unknown';
          if (!overrideLoggedApps.has(appOff)) {
            overrideLoggedApps.add(appOff);
            console.log('[PG-OVERRIDE] app=' + appOff + ' override=off (base passthrough, connection string intacta)');
          }
        } catch (e) { /* best-effort: jamais propagar */ }
        return cb(null, conString);
      }
      const extra = [];
