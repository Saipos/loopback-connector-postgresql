...
class ConnectionParametersWithKeepalive extends BaseConnectionParameters {
  constructor(config) {
    ...
    const idleSeconds = Number(config.keepAliveInitialDelayMillis);
    if (Number.isFinite(idleSeconds)) {
      this.keepalives_idle = Math.floor(idleSeconds / 1000);
    }

    const intervalSeconds = Number(config.keepAliveIntervalMillis);
    if (Number.isFinite(intervalSeconds)) {
      this.keepalives_interval = Math.floor(intervalSeconds / 1000);
    }

    const count = Number(config.keepAliveCount);
  ...
  getLibpqConnectionString(cb) {
    ...
    super.getLibpqConnectionString(function(err, conString) {
      ...
      // TEMP (validacao): loga a connection string resolvida do libpq com a senha mascarada.
      // Remover apos confirmar que os keepalives_* estao chegando.
